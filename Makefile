...
test-vlm-blockchain: $(VLM_DIR)/op-geth/tests/tests.test
	export LD_LIBRARY_PATH=$(LD_LIBRARY_PATH) && cd $(VLM_DIR)/op-geth/tests && ./tests.test -test.run TestULMBlockchain -test.parallel 4 -test.v
...
test-spec-blockchain: $(VLM_DIR)/op-geth/tests/tests.test $(EXECUTION_SPEC_TESTS_DIR)/timestamp
	export LD_LIBRARY_PATH=$(LD_LIBRARY_PATH) && cd $(VLM_DIR)/op-geth/tests && ./tests.test -test.run TestExecutionSpecBlocktests -test.parallel 4 -test.v --skip-spec-block-tests spec-failing.llvm
...
run-failed-spec-blockchain: $(VLM_DIR)/op-geth/tests/tests.test $(EXECUTION_SPEC_TESTS_DIR)/timestamp
	export LD_LIBRARY_PATH=$(LD_LIBRARY_PATH) && cd $(VLM_DIR)/op-geth/tests && ./tests.test -test.run TestExecutionSpecBlocktests -test.parallel 4 -test.v --run-only-spec-block-tests spec-failing.llvm

# Run the Ethereum Conformance Tests
.PHONY: test-blockchain
test-blockchain: $(VLM_DIR)/op-geth/tests/tests.test
	export LD_LIBRARY_PATH=$(LD_LIBRARY_PATH) && cd $(VLM_DIR)/op-geth/tests && ./tests.test -test.run TestBlockchain -test.parallel 4 -test.v --skip-block-tests known-failures.txt
...
test-failed-blockchain: $(VLM_DIR)/op-geth/tests/tests.test
	export LD_LIBRARY_PATH=$(LD_LIBRARY_PATH) && cd $(VLM_DIR)/op-geth/tests && ./tests.test -test.run TestBlockchain -test.parallel 4 -test.v --run-only-block-tests known-failures.txt
