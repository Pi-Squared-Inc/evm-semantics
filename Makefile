...
test-vlm-blockchain: $(VLM_DIR)/op-geth/tests/tests.test
	export LD_LIBRARY_PATH=$(LD_LIBRARY_PATH) && cd $(VLM_DIR)/op-geth/tests && ./tests.test -test.run TestULMBlockchain -test.parallel 8 
...
test-blockchain: $(VLM_DIR)/op-geth/tests/tests.test
	export LD_LIBRARY_PATH=$(LD_LIBRARY_PATH) && cd $(VLM_DIR)/op-geth/tests && ./tests.test -test.run TestBlockchain -test.parallel 8 -test.v --skip-block-tests known-failures.txt
...
test-failed-blockchain: $(VLM_DIR)/op-geth/tests/tests.test
	export LD_LIBRARY_PATH=$(LD_LIBRARY_PATH) && cd $(VLM_DIR)/op-geth/tests && ./tests.test -test.run TestBlockchain -test.parallel 8 -test.v --run-only-block-tests known-failures.txt
