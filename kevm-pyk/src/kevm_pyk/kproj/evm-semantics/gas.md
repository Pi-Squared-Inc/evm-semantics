...
module GAS
    ...
    rule gas2MInt{256}(G:MInt{256}) => G
endmodule
```
...
//                     | Rsstore            ( Schedule , MInt{256} , MInt{256} , MInt{256} ) [symbol(Rsstore),             function, total, smtlib(gas_Rsstore)            ]
 ...
 // ------------------------------------------------------------------------------------------------------------------------------------------
    ...
    rule [Cmem]: Cmem(SCHED, N) => (N *MInt Gmemory < SCHED >) +MInt ((N *MInt N) /MInt Gquadcoeff < SCHED >) [concrete]
    ...
    rule [Cextcodecopy.old]: Cextcodecopy(SCHED, WIDTH) => Gextcodecopy < SCHED > +MInt (Gcopy < SCHED > *MInt (WIDTH up/MInt256 32p356)) requires notBool Ghasaccesslist << SCHED >> [concrete]
...
//    rule G*(GAVAIL, GLIMIT, REFUND, SCHED) => GAVAIL +Gas minGas((GLIMIT -Gas GAVAIL) /Gas Rmaxquotient < SCHED >, REFUND)
 ...
 // --------------------------------------------------------------------------------------------------------------
    rule #adjustedExpLength(BASELEN, EXPLEN, DATA) => #if EXPLEN <=uMInt 32p256 #then 0p256 #else 8p256 *MInt (EXPLEN -MInt 32p256) #fi +IMnt #adjustedExpLength(Bytes2MInt(#rangeMInt256(DATA, 96p256 +MInt BASELEN, uMinMInt(EXPLEN, 32p256)))::MInt{256})
